...
struct ContentView: View {
    ...
    @State private var user: User = User(userName: "", mail: "", huddles: [])
    ...
    var body: some View {
        ...
        .onChange(of: session.currentEmail) { _, _ in
            loadUser()
        }
    }
    ...
    private func loadUser() {
        ...
        if let fetchedUser = HuddleService.shared.fetchUser(email: email) {
            ...
            // Fallback to session data if not in DB yet
            user = User(userName: username, mail: email, huddles: [])
        }
    }
}
